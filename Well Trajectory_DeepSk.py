...
def create_trajectory_pdf(summary_df, detailed_df, params):
    """Generate PDF report of well trajectory"""
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(200, 10, txt="WELL TRAJECTORY REPORT", ln=1, align='C')
    ...
    pdf.cell(200, 10, txt="1. Project Information", ln=1)
    pdf.set_font("Arial", size=10)
    
    project_info = [
        ["Well Name:", params['by_well']],
        ["Field:", params['by_field']],
        ["Company:", params['by_company']],
        ["Surface Location (N/E):", f"{params['surface_northing']:.2f}m / {params['surface_easting']:.2f}m"],
        ["Target Location (N/E):", f"{params['target_northing']:.2f}m / {params['target_easting']:.2f}m"],
        ["Target Depth (TVDSS):", f"{params['target_depth']:.2f}m"],
        ["RKB Elevation:", f"{params['rkb_elevation']:.2f}m"]
    ]
    
    for item in project_info:
        pdf.cell(50, 8, txt=item[0], border=0)
        pdf.cell(0, 8, txt=item[1], ln=1, border=0)
    pdf.ln(5)
    
    # 2. Design Parameters
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(200, 10, txt="2. Design Parameters", ln=1)
    pdf.set_font("Arial", size=10)
    
    design_params = [
        ["Calculation Method:", params['calculation_method']],
        ["KOP Depth:", f"{params['kop']:.2f}m" if params.get('kop') else "N/A"],
        ["Build Rate:", f"{params['bur']:.2f}°/30m" if params.get('bur') else "N/A"],
        ["Target Inclination:", f"{params['target_inc']:.2f}°" if params.get('target_inc') else "N/A"],
        ["Distance to Target:", f"{params['distance_to_target']:.2f}m"]
    ]
    
    for item in design_params:
        pdf.cell(60, 8, txt=item[0], border=0)
        pdf.cell(0, 8, txt=item[1], ln=1, border=0)
    pdf.ln(10)
    
    # 3. Summary Results
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(200, 10, txt="3. Summary Results", ln=1)
    pdf.set_font("Arial", size=8)
    
    # Create summary table
    cols = summary_df.columns
    col_widths = [30, 20, 20, 20, 20, 25, 25, 25, 25]
    ...
    for _, row in summary_df.iterrows():
        for i, col in enumerate(cols):
            if isinstance(row[col], (int, float)):
                ...
                txt = str(row[col])
            pdf.cell(col_widths[i], 8, txt=txt, border=1)
        pdf.ln()
    pdf.ln(15)
    
    # 4. Detailed Survey (first 20 rows)
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(200, 10, txt="4. Detailed Survey Results (Partial)", ln=1)
    pdf.set_font("Arial", size=7)
    
    # Create detailed table
    cols = detailed_df.columns
    col_widths = [20, 20, 15, 15, 15, 15, 25, 25, 20, 20, 15]
    
    # Header
    pdf.set_fill_color(220, 255, 220)
    for i, col in enumerate(cols):
        pdf.cell(col_widths[i], 8, txt=col, border=1, fill=True)
    pdf.ln()
    
    # Rows (first 20 only)
    for _, row in detailed_df.head(20).iterrows():
        for i, col in enumerate(cols):
            if isinstance(row[col], (int, float)):
                txt = f"{row[col]:.2f}"
            else:
    ...
    pdf.cell(0, 10, txt="* Report generated by Well Trajectory Planner", ln=1)
...
if st.session_state.results_calculated:
    ...
    report_params = {
        ...
        'kop': kop if 'kop' in locals() else None,
        'bur': bur if 'bur' in locals() else None,
        'target_inc': target_inc if 'target_inc' in locals() else None,
        'distance_to_target': st.session_state.distance_to_target
    }
    
    # Create PDF
    pdf = create_trajectory_pdf(
    ...
    st.download_button(
        ...
        data=pdf_output,
        file_name=f"Well_Trajectory_Report_{by_well.replace(' ','_')}_{datetime.now().strftime('%Y%m%d')}.pdf",
        mime="application/pdf",
        help="Generate comprehensive PDF report of all trajectory data"
    )
    
    st.info("Report includes: Project info, design parameters, summary results, and partial detailed survey")
